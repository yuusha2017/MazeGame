...
function Role(argX, argY, argZ, ArgImg, ArgSpeed, ArgViewScope, ArgCDT1, ArgCDT2, ArgID, ArgS1Img, ArgS2Img) {
	...
	var ChangeItemDirection = "NoChange";
	...
	this.ChangeItemSelection = function(ArgItemSelection) {		// 改變物品選擇
		var PreItemSelection = ItemSelection;
		ItemSelection += ArgItemSelection;
		if(ChangeItemDirection == "clockwise" && Math.floor(PreItemSelection) != Math.floor(ItemSelection)) {
			ChangeItemDirection = "NoChange";
			ItemSelection = Math.round(ItemSelection);
		}
		else if(ChangeItemDirection == "CounterClockwise" && Math.ceil(PreItemSelection) != Math.ceil(ItemSelection)) {
			ChangeItemDirection = "NoChange";
			ItemSelection = Math.round(ItemSelection);
		}
		ItemSelection = (ItemSelection >= 8) ? (Math.round(ItemSelection)%8) : ItemSelection;
		ItemSelection = (ItemSelection < 0) ? (7.9-(-(Math.round(ItemSelection))%8)) : ItemSelection;
		console.log(ItemSelection);
	};
	this.SetChangeItemDirection = function(direction) {
		ChangeItemDirection = direction;
	};
	...
	this.BasicRoleUpdate = function(progress) {					// 每個角色都有的更新
		if(ChangeItemDirection == "clockwise") {
			this.ChangeItemSelection(5.1*progress);
		}
		else if(ChangeItemDirection == "CounterClockwise") {
			this.ChangeItemSelection(-5.1*progress);
		}
...
// 角色藍結構
function Blue(argX, argY, argZ) {
	Hunter.call(this, argX, argY, argZ, blue, 4, 4, 15, 1, "TreasureHunter", flash, flash);
...
// 角色黒結構
function Black(argX, argY, argZ) {
	Defender.call(this, argX, argY, argZ, black, 7, 4, 20, 20, "TreasureDefender", flash, flash);
