var RoleData = [
	{},
	{name : "Blue", speed : "3", ViewScope : "3", img : blue},
	{name : "unknown", speed : "unknown", ViewScope : "unknown", img : unknown},
	{name : "unknown", speed : "unknown", ViewScope : "unknown", img : unknown},
	{name : "unknown", speed : "unknown", ViewScope : "unknown", img : unknown},
	{name : "unknown", speed : "unknown", ViewScope : "unknown", img : unknown},
	{name : "unknown", speed : "unknown", ViewScope : "unknown", img : unknown},
...
var ItemData = [
	{name : "NoItem", img : unknown},
	{name : "test", img : flash},
	{name : "unknown", img : unknown},
	{name : "unknown", img : unknown},
	{name : "unknown", img : unknown},
	{name : "unknown", img : unknown},
	{name : "unknown", img : unknown},
...
var SelectScene = {
	selection : 0,					// 選擇
	RoleSelection : 1,				// 角色選擇
	ItemSelections : [0,0,0,0],		// 物品選擇
	...
	// 重建選單物件(更新數值與位置)
	RecreateMenuItems : function() {
		this.MenuItems = [new MenuItem("角色  : " + RoleData[this.RoleSelection].name, 	 centerX, GC.height*2.5/7),
						  new MenuItem("物品1 : " + ItemData[this.ItemSelections[0]].name, centerX, GC.height*3.25/7),
						  new MenuItem("物品2 : " + ItemData[this.ItemSelections[1]].name, centerX, GC.height*4/7),
						  new MenuItem("物品3 : " + ItemData[this.ItemSelections[2]].name, centerX, GC.height*4.75/7),
						  new MenuItem("物品4 : " + ItemData[this.ItemSelections[3]].name, centerX, GC.height*5.5/7),
						  new MenuItem("返回", 											 centerX, GC.height*6.25/7)];
	},
	...
	SetRoleSelection : function(RoleSelection) {
		this.RoleSelection = RoleSelection;
	},

	SetItemSelections : function(ItemNum, DataNum) {
		this.ItemSelections[ItemNum-1] = DataNum;
	},
	...
	render : function() {
		...
		GCCT.textAlign = "center";
		GCCT.textBaseline = "middle"; 
		GCCT.strokeText("Check", centerX, GC.height/7);
	...
	// 凸顯選擇的項目
	OnSelect : function() {
		var width = 2*GCCT.measureText("物品1 : NoItem").width;
		var height = 1.2*GCCT.measureText("物").width;
		var text = this.MenuItems[this.selection].name;
		var centerX = this.MenuItems[this.selection].x;
		var centerY = this.MenuItems[this.selection].y;
        GCCT.fillStyle = "AliceBlue";
	...
	// 不凸顯選擇的項目
	UnSelect : function() {
		var width = 2*GCCT.measureText("物品1 : NoItem").width+1;
		var height = 1.2*GCCT.measureText("物").width+1;
		var text = this.MenuItems[this.selection].name;
		var centerX = this.MenuItems[this.selection].x;
		var centerY = this.MenuItems[this.selection].y;
		GCCT.fillStyle = "RGB(210,210,210)";
...
var RoleSelectScene = {
	...
	render : function() {
		...
		for(var x = -1; x < 2; ++x) {
			for(var y = -1; y < 2; ++y) {
				if((x+2) + 3*(y+1) == this.selection) {
					GCCT.fillStyle = "yellow";
					GCCT.fillRect(GC.width/2+x*GC.height/6-GC.height/12,3*GC.height/4 + y*GC.height/6 - GC.height/12 - GC.height/24,GC.height/6,GC.height/6);
				}
				GCCT.rect(GC.width/2+x*GC.height/6-GC.height/12,3*GC.height/4 + y*GC.height/6 - GC.height/12 - GC.height/24,GC.height/6,GC.height/6);
				GCCT.drawImage(RoleData[(x+2) + 3*(y+1)].img, GC.width/2+x*GC.height/6-GC.height/12,3*GC.height/4 + y*GC.height/6 - GC.height/12 - GC.height/24, GC.height/6,GC.height/6);
			}
		}
		GCCT.stroke();

		// 畫所選角色
		GCCT.drawImage(RoleData[this.selection].img, GC.width/2 - GC.height/12, GC.height/4 - GC.height/12, GC.height/6,GC.height/6);
	},

	// 上鍵處理函式
	Up : function() {
		this.selection = (this.selection - 3 > 0) ? (this.selection - 3) : this.selection+6;
		this.render();
	},

	// 下鍵處理函式
	Down : function() {
		this.selection = (this.selection + 3 < 10) ? this.selection + 3 : this.selection-6;
		this.render();
	},

	// 右鍵處理函式
	Right : function() {
		this.selection = ((this.selection + 1) % 3 != 1) ? this.selection + 1 : this.selection-2;
		this.render();
	},

	// 左鍵處理函式
	Left : function() {
		this.selection = ((this.selection - 1) % 3 != 0) ? this.selection - 1 : this.selection+2;
		this.render();
	}
};
...
var ItemSelectScene = {
	...
	render : function() {
		...
		for(var x = -2; x < 3; ++x) {
			for(var y = -2; y < 3; ++y) {
				if((x+3) + 5*(y+2) == this.selection) {
					GCCT.fillStyle = "yellow";
					GCCT.fillRect(GC.width/2+x*GC.height/10-GC.height/20,3*GC.height/4 + y*GC.height/10 - GC.height/20 - GC.height/40,GC.height/10,GC.height/10);
				}
				GCCT.rect(GC.width/2+x*GC.height/10-GC.height/20,3*GC.height/4 + y*GC.height/10 - GC.height/20 - GC.height/40,GC.height/10,GC.height/10);
				GCCT.drawImage(ItemData[(x+3) + 5*(y+2)].img, GC.width/2+x*GC.height/10-GC.height/20,3*GC.height/4 + y*GC.height/10 - GC.height/20 - GC.height/40, GC.height/10,GC.height/10);
			}
		}
		GCCT.stroke();

		// 畫所選物品
		GCCT.drawImage(ItemData[this.selection].img, GC.width/2 - GC.height/20, GC.height/4 - GC.height/20, GC.height/10,GC.height/10);
	},

	// 上鍵處理函式
	Up : function() {
		this.selection = (this.selection - 5 > 0) ? (this.selection - 5) : this.selection+20;
		this.render();
	},

	// 下鍵處理函式
	Down : function() {
		this.selection = (this.selection + 5 < 26) ? (this.selection + 5) : this.selection-20;
		this.render();
	},

	// 右鍵處理函式
	Right : function() {
		this.selection = ((this.selection + 1) % 5 != 1) ? this.selection + 1 : this.selection-4;
		this.render();
	},

	// 左鍵處理函式
	Left : function() {
		this.selection = ((this.selection - 1) % 5 != 0) ? this.selection - 1 : this.selection+4;
		this.render();
	}
};
	...
	UpdateViewScope : function(role) {
		...
		if(role.GetState() != "vanish") {
			...
			for(var i = -2; i <= 5; ++i) {
				...
				GCCT.drawImage(ItemBorder, -this.FixedSL/2 + (this.SL*(RoleViewScope) + this.FixedSL/3)*Math.cos(2*Math.PI/8*i + role.GetItemSelection()/8*2*Math.PI), -this.FixedSL/2 + (this.SL*(RoleViewScope) + this.FixedSL/3 )*Math.sin(2*Math.PI/8*i + role.GetItemSelection()/8*2*Math.PI), this.FixedSL, this.FixedSL);
			}

			// 玩家擁有物品繪製
			for(var i = -2; i <= 5; ++i) {
				if(role.GetItem(i+2) != "NoItem") {
					if(i+2 == role.GetItemSelection()) {
						GCCT.globalAlpha = 1;
					}
					else {
						GCCT.globalAlpha = 0.2;
			...
			// 		45*role.GetItemSelection() += 5;
			// 	}
			// 	else {
			// 		45*role.GetItemSelection() -= 5;
			// 	}
			// 	if(45*role.GetItemSelection() % 45 == 0) {
			// 		this.ChangeItemAnimationRequest = false;
			// 		if(45*role.GetItemSelection() % 360 == 0) {
			// 			45*role.GetItemSelection() = 0;
			// 		}
			// 	}
			// }
